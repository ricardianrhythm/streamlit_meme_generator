...
def collect_ip():
    ...
    print(f"Collected IP: {user_ip}, Location: {location}, Thought: {thought}")
    
    # Return the IP address to be stored in Firebase
    return jsonify({"ip_address": user_ip})

def run_flask():
    # Run the Flask app in debug mode without the reloader to avoid Jupyter conflicts
...
# Use the locally hosted Flask server URL
FLASK_SERVER_URL = "http://127.0.0.1:5000/collect_ip"
...
def collect_user_ip(location_label, thought):
    """
    Send a request to the Flask server to collect the user's IP address.
    """
    try:
        response = requests.get(FLASK_SERVER_URL, params={"location": location_label, "thought": thought})
        if response.status_code == 200:
            ip_address = response.json().get("ip_address")
            print(f"IP collected successfully: {ip_address}")  # Debugging statement
            return ip_address
        else:
            print(f"Failed to collect IP. Status code: {response.status_code}, Response: {response.text}")
            return None
    except Exception as e:
        print(f"Error while collecting IP: {e}")
        return Nonef
